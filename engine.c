        ...
        while (rec) {
            if (rec->host_pid == pid) {
                if (WIFEXITED(status)) {
                    ...
                    if (rec->exit_signal == SIGKILL && !ctx->stop_requested) {
...
static void *producer_thread(void *arg)
{
    ...
    char container_id[CONTAINER_ID_LEN];
    strncpy(container_id, parg->container_id, sizeof(container_id) - 1);
    ...
    while (1) {
        ...
        item.data[n] = '\0';
        item.length = n;
        strncpy(item.container_id, container_id, sizeof(item.container_id) - 1);
...
static int handle_start_or_run(supervisor_ctx_t *ctx, const control_request_t *req, int is_run)
{
    int pipe_fds[2];
    pid_t child_pid;
    char *stack;
    ...
    if (parg) {
        ...
        parg->container_id[sizeof(parg->container_id) - 1] = '\0';

        pthread_t prod_thread;
        pthread_create(&prod_thread, NULL, producer_thread, parg);
...
static int run_supervisor(const char *rootfs)
{
    ...
    while (!ctx.should_stop) {
        ...
        if (req.kind == CMD_START) {
            if (handle_start_or_run(&ctx, &req, 0) == 0) {
                resp.status = 0;
            ...
            if (handle_start_or_run(&ctx, &req, 1) == 0) {
                resp.status = 0;
                snprintf(resp.message, sizeof(resp.message), "Container %s completed", req.container_id);
            } else {
                resp.status = -1;
            ...
            while (rec) {
                char line[256];
                snprintf(line, sizeof(line), "%s\t%d\t%s\tsoft=%luMB hard=%luMB\n",
            ...
            if (rec && rec->state == CONTAINER_RUNNING) {
                ...
                resp.status = 0;
                snprintf(resp.message, sizeof(resp.message), "Stopping container %s", req.container_id);
            } else {
                resp.status = -1;
        ...
        ssize_t written = write(client_fd, &resp, sizeof(resp));
        (void)written;
        close(client_fd);
    }
...
static int send_control_request(const control_request_t *req)
{
    ...
    if (req->kind == CMD_LOGS && resp.status == 0) {
        int fd = open(resp.message, O_RDONLY);
        if (fd >= 0) {
            char buf[4096];
            ssize_t bytes;
