 ...
 *
 * This module tracks memory usage (RSS) of registered processes
 * and enforces soft and hard memory limits.
 *
 * Features:
 *   - Periodic monitoring using kernel timer
 *   - Soft limit warning (logged once per process)
 *   - Hard limit enforcement (SIGKILL)
 *   - Dynamic registration/unregistration via ioctl
 */
...
/* ============================
 * Monitored Process Structure
 * ============================ */
struct monitored_entry {
    pid_t pid;                              // Process ID
    char container_id[MONITOR_NAME_LEN];    // Container identifier
    unsigned long soft_limit_bytes;         // Soft memory limit
    unsigned long hard_limit_bytes;         // Hard memory limit
    int soft_limit_warned;                  // Flag to avoid duplicate warnings
 ...
 * Global State
 * ============================ */
static LIST_HEAD(monitored_list);   // List of monitored processes
static DEFINE_MUTEX(monitored_lock); // Protects list access
 ...
 * RSS Helper
 * ============================ */
/* Returns RSS in bytes, or -1 if process does not exist */
...
/*
 * Periodically:
 *   - Checks RSS of each process
 *   - Removes dead processes
 *   - Logs soft-limit violation (once)
 *   - Kills process on hard-limit violation
 */
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        /* Trigger soft limit warning (only once) */
